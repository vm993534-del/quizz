...
var initialSrc = "https://lp.sistemaof.com/"+campaignID+"?format=json";
...
setTimeout(function(){
	if (!urlParams.get('rtkcid')) {
		...
		xhr.onreadystatechange = function() {
			if (xhr.readyState == 4 && xhr.status == 200) {
				...
				document.querySelectorAll('a').forEach(function(el) {
					if (el.href.indexOf("lp.sistemaof.com/preclick")>-1) {
				...
				xhrr.open("GET", "https://lp.sistemaof.com/preview?clickid="+rawData.clickid)
				xhrr.send();
			}
		}
		...
		xhrTrack.open("GET", "https://lp.sistemaof.com/preview?clickid="+rtkClickID )
		xhrTrack.send();
		document.querySelectorAll('a').forEach(function(el) {
			if (el.href.indexOf("lp.sistemaof.com/preclick")>-1) {
				if (el.href.indexOf('?')>-1) {
